...
export const SessionOutline = memo(function SessionOutline({
  ...
  const items = useMemo(
    () =>
      entries.map((e, i) => {
        const one = e.text.replace(/\s+/g, " ").trim();
        const max = collapsed ? 96 : 42;
        const preview = !one
          ? t("outline.empty")
          : one.length <= max
            ? one
    ...
    >
      <div className="session-outline-rail">
        <button
          ...
          title={collapsed ? t("outline.expand") : t("outline.collapse")}
        ...
        >
          ...
          {!collapsed && (
            <span className="session-outline-toggle-label">
              {t("outline.inputs", { n: items.length })}
            </span>
          )}
        </button>
