...
//!
//! Product model:
//! - **Project** = concrete workspace path (stable, user-chosen) = agent cwd
//! - **Session** (UI: Task) = chat metadata under `<project>/.grokx/tasks/<id>/`
//!   (legacy installs may still use `~/.grokx/tasks/<id>/`)
//!
//! Persistence:
//! - App data: `sessions-index.json` (project + session list)
//! - Per task: `<project>/.grokx/tasks/<id>/meta.json` + `chat-history.json`
...
pub struct SessionListItem {
    ...
    pub project_root: String,
    pub project_name: String,
    /// Task metadata dir (`<project>/.grokx/tasks/<id>`; legacy: `~/.grokx/tasks/<id>`).
    pub work_path: String,
...
impl SessionStore {
    ...
    /// Scan a tasks root (`~/.grokx/tasks` or `<project>/.grokx/tasks`) and
    /// import any tasks missing from the index.
        ...
        for entry in rd.flatten() {
            ...
            let (project_root, project_name, title, model, created_at, updated_at, project_id) =
                if meta_path.is_file() {
                    ...
                    // Infer from project pointer file + chat mtime.
                    let project_link = path.join("project");
                    let project_root = std::fs::read_to_string(&project_link)
                        .ok()
                        .map(|s| s.trim().to_string())
                        .filter(|s| !s.is_empty())
                        .or_else(|| {
                            std::fs::read_link(&project_link)
                                .ok()
