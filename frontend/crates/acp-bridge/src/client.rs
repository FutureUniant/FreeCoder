...
use crate::permission_gate::{
    permission_outcome_value_with_options, ParkedPermission, PermissionGate,
};
...
impl AcpClientHandle {
    async fn handshake(&self) -> Result<(), BridgeError> {
        let init = self
            .request(
                "initialize",
                // Identify as grok-desktop so the agent builds Desktop permission
                // option sets (`allow-always-mcp`, …). Without this, ClientType
                // stays Generic and "Allow similar" can send option ids the
                // agent does not recognize → tool fails after the user clicks Allow.
                json!({
                    "protocolVersion": 1,
                    "clientInfo": {
                        "name": "grok-desktop",
        ...
        // Local / custom endpoints inject XAI_API_KEY (even a placeholder like
        // "local"). Prefer API-key auth and never fall through to browser
        // `grok.com` OIDC — that hangs Connecting for minutes on machines
        // without an X account / with blocked auth.x.ai.
        let api_key_method = auth_methods.iter().find_map(|method| {
            let id = method.get("id").and_then(|v| v.as_str()).unwrap_or("");
            if id == "xai.api_key" || id.contains("api_key") {
                Some(id.to_string())
            } else {
                None
            }
        });
        if let Some(id) = api_key_method {
            let _ = self
        ...
        if let Err(BridgeError::Rpc { message, .. }) = &result {
            if message.to_lowercase().contains("auth") && !auth_methods.is_empty() {
                for method in &auth_methods {
                    ...
                    // Skip browser/OIDC login — it blocks the desktop UI.
                    if id == "grok.com" || id.contains("oidc") || id.contains("oauth") {
                        warn!(method = id, "skipping interactive auth method during ACP handshake");
                        continue;
                    }
                    let _ = self
                        .request("authenticate", json!({ "methodId": id }))
    ...
    pub async fn prompt_request(&self, req: PromptRequest) -> Result<(), BridgeError> {
        ...
        match result {
            ...
            Err(err) => {
                // Do not emit AgentError / TurnFinished here — the caller
                // (app-core) classifies truncation vs hard failure and owns UX
                // (auto-continue must not flash "Turn ended with error").
                let _ = app_session_id;
                Err(err)
            }
        }
    }
    ...
    pub async fn cancel(&self) -> Result<(), BridgeError> {
        ...
        // 2) Deny parked tool-permission waits *and* write ACP responses.
        //    Dropping the gate alone leaves the engine blocked on RPC replies
        //    (zombie `task_already_running` after Stop).
        let denied: Vec<(Value, Value)> = {
            let mut gate = self.shared.permission_gate.lock().await;
            gate.pending_ids()
                .into_iter()
                .filter_map(|id| {
                    gate.resolve(&id, PermissionDecision::Deny)
                        .ok()
                        .map(|(rpc_id, outcome)| (rpc_id, outcome))
                })
                .collect()
        };
        for (rpc_id, outcome) in denied {
            let _ = write_response(&self.shared, rpc_id, outcome).await;
        }
...
async fn handle_incoming(shared: Arc<Shared>, value: Value) -> Result<(), BridgeError> {
            ...
            if let Some(pending) = shared.pending.lock().await.remove(&id) {
                let result = if let Some(err) = value.get("error") {
                    ...
                    let message = data
                        ...
                        .or_else(|| data.and_then(|d| d.as_str()))
                        .unwrap_or("unknown error")
                        .to_string();
                    let app_code = data
                        .and_then(|d| d.get("app_code"))
                        .and_then(|c| c.as_str())
                        .map(|s| s.to_string());
                    Err(BridgeError::Rpc {
                        code,
    ...
    match method_norm {
        ...
        // Grok Build extension rail: GoalUpdated, WorkflowUpdated, …
        ...
        "session/request_permission" | "request_permission" => {
            ...
            let (similarity_key, options) =
                if let AppEvent::PermissionNeeded { request, .. } = &event {
                    (request.similarity_key.clone(), request.options.clone())
                } else {
                    (
                        crate::map::permission_similarity_key(&params, &tool_name),
                        Vec::new(),
                    )
                };
            ...
            if !PermissionGate::should_park(shared.options.auto_approve) {
                let _ = shared.events.send(event);
                let result = permission_outcome_value_with_options(
            ...
            // Bailian media generate_* is product core path — never block on ask.
            // Users were stuck in: wait → allow → "unknown permission option" → no image.
            if is_trusted_media_generation(&tool_name, &summary) {
                {
                    let mut gate = shared.permission_gate.lock().await;
                    gate.remember_session_allow(&similarity_key);
                }
                let result = permission_outcome_value_with_options(
                    PermissionDecision::AllowOnce,
                    &options,
            ...
            // Session "allow similar" — answer without prompting again.
            {
                let gate = shared.permission_gate.lock().await;
                if gate.session_allowed(&similarity_key) {
                    drop(gate);
                    let result = permission_outcome_value_with_options(
                        PermissionDecision::AllowOnce,
                        &options,
...
/// Image/video generation via Bailian media MCP must not wait for ask-mode
/// approval. Permission option-id mismatches were failing the tool after the
/// user clicked Allow, producing the "图片呢？" loop.
fn is_trusted_media_generation(tool_name: &str, summary: &str) -> bool {
    let hay = format!("{tool_name} {summary}").to_ascii_lowercase();
    hay.contains("generate_image")
        || hay.contains("generate_video")
        || hay.contains("bailian-media")
}

fn build_prompt_blocks(req: &PromptRequest) -> Result<Vec<Value>, BridgeError> {
    let mut blocks = Vec::new();
    let text = sanitize_leading_prompt_text(req.text.trim());
    if !text.is_empty() {
    ...
    for att in &req.attachments {
        ...
        // Folder path chips: tell the agent the directory path (do not ingest tree).
        if att.is_dir || path.is_dir() {
            if !path.is_dir() {
            ...
            folder_notes.push(att.path.clone());
            continue;
        }
        if !path.is_file() {
            return Err(BridgeError::Message(format!(
    ...
    if !folder_notes.is_empty() {
        blocks.push(json!({
            "type": "text",
            "text": format!(
                "Attached folder path(s) — treat each as a directory reference; explore on disk as needed:\n{}",
                folder_notes
                    .iter()
                    .map(|s| format!("- `{s}`"))
                    .collect::<Vec<_>>()
    ...
    // Ensure we always have at least a path note if only binary files / folders.
...
/// Strip BOM / ZWSP so a leading `/goal` stays visible to the engine slash parser.
fn sanitize_leading_prompt_text(text: &str) -> String {
...
mod tests {
    ...
    #[test]
    fn sanitize_keeps_slash_goal_leading() {
        assert_eq!(
            sanitize_leading_prompt_text("\u{200b}/goal do the thing"),
            "/goal do the thing"
        );
    }
