...
pub fn resolve_engine(
    ...
    if settings.prefer_bundled_engine {
        ...
        // Downloaded / install-dir engine (resources/runtime/grok.exe under install root).
        if let Ok(paths) = app_config::AppPaths::discover() {
            candidates.push(paths.downloaded_grok_exe());
        }
        candidates.push(app_config::AppPaths::product_install_root().join(
            if cfg!(windows) {
                "resources/runtime/grok.exe"
            } else {
                "resources/runtime/grok"
            },
        ));
