...
impl PermissionBroker {
    ...
    pub fn resolve(
        ...
        if decision == PermissionDecision::AllowSession {
            let key = if !req.similarity_key.trim().is_empty() {
                req.similarity_key.clone()
            } else {
                req.tool_name.clone()
            };
            self.session_allows.insert(key, true);
        }
        Ok(req)
    }

    pub fn session_allowed(&self, key: &str) -> bool {
        let k = key.trim();
        !k.is_empty() && self.session_allows.get(k).copied().unwrap_or(false)
    }
