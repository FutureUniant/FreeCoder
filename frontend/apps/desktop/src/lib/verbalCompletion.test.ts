...
check("已生成成功!现在下载… trailing colon is verbal", () => {
  assert(
    isVerbalProgressOnly(
      "视频已经生成成功！现在让我把它下载到工作区：",
    ),
    "success claim + download promise must still be verbal",
  );
});

check("claimed success without path → nudge", () => {
  const lines: VerbalChatLine[] = [
    { kind: "user", text: "生成了吗？" },
    {
      kind: "trace",
      items: [
        { id: "t1", kind: "tool", text: "generate_video" },
        { id: "t2", kind: "tool", text: "generate_video" },
      ],
    },
    {
      kind: "assistant",
      text: "视频已经生成成功！现在让我把它下载到工作区：",
    },
  ];
  const hit = detectVerbalOnlyCompletion(lines);
  assert(hit, "expected hit for success-without-path");
...
check("(c) who are you + short answer → no hit", () => {
  const lines: VerbalChatLine[] = [
    ...
    { kind: "assistant", text: "I am FreeCoder, a desktop coding assistant." },
  ];
  const hit = detectVerbalOnlyCompletion(lines);
